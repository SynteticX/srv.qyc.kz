...
// Получить все строки по параметрам
function get_rows_from_table($table, $params) {
    require "config.php";

    // Пример использования:
    // $rows = get_rows_from_table("forms", ["status" => "active"]);
    ...
    foreach ($params as $key => $value) {
        $sql_conditions[] = "`$key` = ?";
        $bind_types .= is_integer($value) ? 'i' : 's'; // предполагаем, что значения только строки и целые числа
        $bind_values[] = $value;
    }

    // SQL запрос для поиска строк
    $sql = "SELECT * FROM `$table` WHERE " . implode(' AND ', $sql_conditions);
    ...
    if ($stmt = mysqli_prepare($link, $sql)) {
        ...
        if (mysqli_stmt_execute($stmt)) {
            ...
            while ($row = mysqli_fetch_assoc($result)) {
                $rows[] = $row; // Добавляем каждую найденную строку в массив
            }
            
            // Возвращаем массив строк, если он не пустой
            if (!empty($rows)) {
                return $rows;
            }
        } else {
    ...
    // Закрываем соединение
    mysqli_close($link);
    return false; // В случае, если строки не найдены
}
